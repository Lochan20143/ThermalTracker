# --- vision_core.py (Final Optimised with full-frame fallback and debugging) ---
import cv2
import numpy as np
...
def generate_frames():
    ...
    while True:
        ...
        # Apply contrast enhancement and sharpening (for phone screen detection)
        gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
        clahe = cv2.createCLAHE(clipLimit=3.0, tileGridSize=(8, 8))
        cl = clahe.apply(gray)
        sharp = cv2.Laplacian(cl, cv2.CV_64F)
        sharpened = cv2.convertScaleAbs(cl + 0.5 * sharp)
        frame = cv2.cvtColor(sharpened, cv2.COLOR_GRAY2BGR)

        # Simulated IR
        if IR_MODE:
            display = cv2.applyColorMap(sharpened, cv2.COLORMAP_INFERNO)
        else:
            display = frame.copy()
        ...
        if len(results.boxes) == 0:
            # Fallback: MobileNet classification on full frame
            pil_crop = Image.fromarray(cv2.cvtColor(frame, cv2.COLOR_BGR2RGB))
            label, score = classify_image_pil(pil_crop)
            print(f"[MobileNet Fallback] {label} - {score}%")
            cv2.putText(annotated, f"ImageNet: {label} ({score}%)", (20, 40),
